          ...
          <div className="md:flex-1">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div className="flex items-center gap-3 border border-gray-200 rounded-full px-3 py-1">
                <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="text-gray-500 text-[18px] font-medium hover:text-[#D65A5A] active:scale-90 transition-all">−</button>
                <span className="text-[14px] font-semibold w-5 text-center">{quantity}</span>
                <button onClick={() => setQuantity((q) => Math.min(maxStock, q + 1))} className="text-gray-500 text-[18px] font-medium hover:text-[#D65A5A] active:scale-90 transition-all">+</button>
              </div>
            </div>
            <div className="hidden md:flex gap-3">
              <button onClick={handleAddToCart} className="flex-1 border border-[#D65A5A] text-[#D65A5A] font-semibold py-3 rounded-full text-[14px] hover:bg-[#fff0f0] active:scale-95 transition-all">Add to Cart</button>
              <button onClick={handleBuyNow} className="flex-1 bg-[#D65A5A] text-white font-semibold py-3 rounded-full text-[14px] hover:bg-[#c44f4f] active:scale-95 transition-all">Buy Now</button>
            </div>
          </div>
        </div>
      </div>
